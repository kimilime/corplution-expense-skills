...
*Skill**确保输出的Excel及文件包符合报销标准，根据用户输入的信息执行推断和匹配，并替报销人撰写所有的Note。信息越详细，匹配越准确，若原始记录详细程度不足，你可以自由通过对话提出要求（如：5.10-5.22皆在CORP-2026-XXXX 北京市的AAA客户项目上，请按照开票地和报销标准自行推测和匹配我提供的全部餐费发票，此外该客户办公地点为海淀中关村，因此滴滴行程中该地点为“客户”，其他无法确定的再问我），由模型在规范内执行先行推断和匹配工作，再进行检查和调整，以最大限度减少手工工作。*
...
- **特殊 / 一次性餐标**：某个事件当天的实际标准与公司通用标准不同时，直接说。例如年会/半年会“自理餐标 7 月 17 日 60 元、7 月 18 日 150 元”。这类一次性标准只写进该事件的项目上下文（不改动公司通用政策），并只在匹配的项目与日期上生效；不高于通用标准的凭你的说明放行，高于的会照办但作提示，便于审批人知晓。
