...
| Subagent result rejects `coverage.status`, `check_id`, `references`, or `summary` / 子 Agent 返回格式被拒 | Start from the generated result template and use the task's `response_json_schema` when the host supports it. `coverage[].status` is only `completed` or `not_applicable`; the audit's overall conclusion belongs in `outcome`, and every finding has exactly `finding_id`, `severity`, `code`, `message`, `unit_refs`, `evidence_refs`, and `recommended_action`. Do not hand-edit a rejected result into compliance without rerunning the fresh audit. / 从结果模板开始；支持结构化输出时使用任务携带的 JSON Schema。 |
| Subagent audit found issues / 子 Agent 审计有发现 | Findings are advisory unless `severity: blocking`; a blocking finding from either Mirror Warden or Gate Challenger gates Stage 3. Resolve cited items through Composer/Updater and rerun the audit — never hand-edit an accepted result. / 阻断项须经 Composer/Updater 正规修正并重跑审计，不得手改结果。 |
