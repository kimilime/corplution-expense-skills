...
| No subagent runtime is available / 宿主不支持子 Agent | Skip the optional pilot and continue with the existing deterministic workflow. Missing/unavailable review is reported as fail-open, never as `pass`. / 跳过试验层，继续原流程；只能标记未运行/不可用，不能伪造通过。 |
