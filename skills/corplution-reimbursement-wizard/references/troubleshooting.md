...
| Subagent result is rejected as stale or has an unknown `N@ref` / 子 Agent 结果代际或引用失效 | Discard the result, rerun Chief `prepare-agent` against the CURRENT allocation, and hand the fresh compact task/template to a fresh subagent through a read-only host attachment/resource when available; otherwise include both complete JSON values in its initial message. Never replace only the fingerprint or refs, or ask it to locate workspace files. / 丢弃旧结果，从当前 allocation 重新生成紧凑任务；优先通过只读附件/资源交接，禁止只换指纹或编号，也不要让子 Agent 自己找工作区文件。 |
| Subagent result rejects `coverage.status`, finding `code`/severity, references, or evidence rule / 子 Agent 返回格式或证据规则被拒 | Start from the generated result template and current task contract; use `response_json_schema` when supported. Each role has its own code vocabulary and minimum evidence rules: do not rename a rejected code, add dummy refs, or hand-edit the result into compliance. Rerun a fresh audit from the unchanged canonical packet. / 从当前模板和任务契约重新启动独立审计；禁止给被拒结果换 code、补假引用或手工修成可验收格式。 |
| Subagent audit found issues / 子 Agent 审计有发现 | Relay the generated `SUBAGENT REVIEW SUMMARY TO SHOW IN CHAT` verbatim. Only `severity: blocking` enters the applicant question loop and gates Stage 3. `advisory` is information only: do not ask for a choice, do not change allocation, and continue by default. / 原样转发脚本摘要；仅阻断项需要申请人回答，advisory 无需回复且不得升级成待决问题。 |
| Subagent audit blocks Stage 3 / 子 Agent 审计阻断写表 | Resolve each cited item through Composer/Updater. The allocation fingerprint then changes, so rerun the audit before Stage 3. Accepted results are retained in an immutable per-role generation archive; deleting or editing the sidecar cannot clear the blocker. / 按引用项正规修正，随后对新代际重新审计；受理结果另有不可变世代归档，删改 sidecar 不能解除阻断。 |
| A blocker is answered by a durable business fact / 阻断项由业务事实解释 | Persist the fact through Composer/Updater in the relevant context `user_notes` or item `expense_note`, then rerun a fresh audit. Example: company Ctrip Business Travel booked the contextual flight, so the employee neither claims nor issues a personal flight invoice. Never leave a fact needed by a fresh subagent only in chat. / 将“公司商旅统一采购、个人不报机票”等事实写入上下文或费用备注后重审，不能只留在聊天记忆中。 |
