...
## Optional Otako Allocation Pass

When the host can create a fresh isolated subagent, run Chief `prepare-agent --role allocation_analyst` only after the current allocation and any required rebase are settled. The generated task is a path-free snapshot containing role instructions, current units/questions/hints, evidence summaries, policy, and full allocation/extraction fingerprints. Pass the complete task and result-template JSON values to the subagent, not their local paths, and do not grant mutation tools.
