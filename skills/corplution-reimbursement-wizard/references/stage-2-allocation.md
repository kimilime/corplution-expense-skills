...
## Preferred Otako Allocation Pass

When the host can create a fresh isolated subagent, run Chief `prepare-agent --role allocation_analyst` by default after the current allocation and any required rebase are settled, before relaying unresolved questions. Skip only if the capability is unavailable or the user opts out. The generated task is a compact role-scoped, path-free snapshot containing role instructions, current units/questions/hints, evidence summaries, policy, and full allocation/extraction fingerprints; it is capped at 80 KiB. Pass the task and result-template JSON through a host read-only attachment/resource when available; otherwise put both complete JSON values in the fresh subagent's initial message. Do not give it local paths, ask it to locate workspace files, or grant mutation tools. When the host supports structured output, use the task's `response_json_schema`.

Validate the returned `allocation_analysis.v2` through Chief `accept-agent`. Every `coverage[]` entry must contain only `check_id`, `status`, and `notes`, and `status` is only `completed` or `not_applicable`; allocation advice belongs in `proposals` or `user_questions`. Every proposal must contain exactly `proposal_id`, `unit_refs`, `set`, `confidence`, `reason`, and `evidence_refs`; every user question must contain exactly `question_id`, `unit_refs`, `question`, `reason`, and `blocking`. Validation also requires exact current `N@ref` tokens, updater-supported proposal fields, and the full 64-character allocation fingerprint. The accepted analysis lives outside `process/` and cannot alter Stage 2 readiness. Its generated `.unreviewed.json` packet is structurally rejected by Composer. Review proposal IDs against the applicant's statements, run Chief `promote-proposals --select ... --reviewed-by coordinator|applicant`, then pass only the resulting stamped `.reviewed.json` file to Composer with `--proposal`. Proposal mode rejects abbreviated generation fingerprints, changed selections, and proposals whose source analysis is no longer current.
