...
- Flight: ordinary/special VAT invoices may still be flight evidence. Preserve line items such as `国内航空`, `国际航空`, `航空运输`, or `航空旅客运输`, plus seller and remark evidence, so Stage 2 can recognize the flight without a dedicated document subtype. Airline name is evidence, not the final reimbursement Note.
...
- Before matching individual railway tickets, group only tightly connected ticket segments into a railway journey chain when both departure timestamps are usable, the later departure is no more than six hours after the earlier departure, and one segment's destination station/city matches the next segment's origin station/city. Missing times do not auto-form a chain. Explicit per-leg project declarations break the chain. When an intermediate city has an active project context for the travel date, ask whether it was merely a transfer or an actual project stop instead of auto-assigning the whole chain to the terminal project.
...
- Show automatically inferred railway chains as one non-blocking review line in chat. Ask one whole-chain question when the chain does not point to a unique project, the station/time sequence is ambiguous, an intermediate city has an active project context, or the user says an intermediate city was an actual stop/project rather than a transfer.
...
- Treat the workbook, `final-expense-rows.json`, and `final-expense-rows.md` as one Stage-3 generation: stage and validate all three before promotion, restore the prior set on a promotion failure, and never delete the prior final rows merely because a new write attempt is blocked. Only `STAGE3_RESULT: ok` with exit code `0` and `package_allowed=true` authorizes Stage 4; `review_required` writes review artifacts but remains non-packageable.
...
- End with the generated Close Message only when the package manifest has no issues. Read `references/close-message.md` and relay its `CLOSE MESSAGE TO SHOW IN CHAT` block verbatim so package/workbook locations, evidence counts, amount changes, exclusions/omissions with reasons, and project totals are never lost. If the manifest has issues, list them directly in chat as blocking items, ask for the missing file or decision, and re-run Stage 4 after resolution.
...
- The successful package manifest contains a stamped `reimbursement_close_summary.v1`, and its generated Close Message was relayed verbatim; no total, exclusion reason, amount adjustment, not-reimbursed applicant record, or project summary was reconstructed from memory.
