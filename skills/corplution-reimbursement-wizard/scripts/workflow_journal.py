...
def snapshot_artifacts(process_dir: str | Path, output_root: str | Path) -> dict[str, Any]:
    ...
    specs = [
        ...
        ("mirror_warden_audit", pdir / "mirror_warden-audit.json", {
            "findings": "finding_count",
        }),
        ("gate_challenger_audit", pdir / "gate_challenger-audit.json", {
            "findings": "finding_count",
        }),
    ]
    ...
    audit_archives = sorted(
        (path for path in (pdir / "subagent-audit-generations").glob("*/*/*.json") if path.is_file()),
        key=lambda item: item.stat().st_mtime_ns,
    )
    if audit_archives:
        snapshot["subagent_audit_archive"] = {
            "accepted_result_count": len(audit_archives),
            "latest_file_sha256": sha256_file(audit_archives[-1]),
        }
