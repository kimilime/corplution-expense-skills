...
5. 用户回答后，优先使用 Composer 将自然语言决议转换为当前 allocation 专属的 canonical `unit_updates`，自动 dry-run 后再交给 updater 应用；仅在 Composer 无法表达的恢复场景使用模板生成器，不要凭空编造 `answers[].allocations` 等 JSON 结构。
...
# Recovery/schema inspection only; Composer is the default answers path.
python skills/corplution-reimbursement-wizard/scripts/chief_orchestrator.py run template
...
原有脚本仍可直接调用，用于调试和恢复，但只有经 Chief 调度的运行会写入工作流日志。
...
- 归集记录提取批次指纹；如果重新提取了材料，必须重新归集、重新编译 answers 并重新写表，不能修补或复用旧文件继续使用。
...
- Allocation records its extraction generation. After re-extraction, rerun allocation, recompose answers, and rewrite the workbook; do not patch or replay older process/answers files.
...
5. After the user answers, use Composer by default to convert decisions into the current allocation's canonical `unit_updates`, dry-run them automatically, and apply them through the updater. Use the template generator only for Composer-inexpressible recovery cases; do not invent JSON shapes such as `answers[].allocations`.
...
The original scripts remain directly callable for debugging and recovery, but only Chief-dispatched runs are journaled.
