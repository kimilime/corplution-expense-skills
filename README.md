...
5. 用户回答后，将自然语言决议写入 `allocation_decisions.v1`，再由 Composer 转换为当前 allocation 专属的 canonical `unit_updates`，自动 dry-run 后交给 updater 应用。Composer 支持 unit/question/context 更新和 confirm/drop/exclude；失败时修正同一份 decisions 文件重跑，不得改用模板填充、`fill_answers.py` 或 patch/fix 脚本。
...
# --set is for short ASCII values only; use --decisions for Chinese/complex text.
python skills/corplution-reimbursement-wizard/scripts/chief_orchestrator.py run compose \
  --set "3,5-7: status=confirmed"
...
- `expense_hint_reconciliation` 逐条记录用户费用说明的匹配或明确处理结果；只要还有一条没有对应凭证、合并覆盖、补票或不报销决定，Stage 3 和打包就会阻断。
...
- Allocation records both the extraction generation and the project-context SHA-256. After either input changes, disappears, or becomes invalid, rerun allocation, recompose answers, and rewrite the workbook; do not patch or replay older process/answers files.
- `expense_hint_reconciliation` records the outcome for every applicant expense note. Stage 3 and packaging stop while any record lacks a matched item, combined-invoice, later-supplement, or not-reimbursed decision.
...
5. After the user answers, write `allocation_decisions.v1` and use Composer to convert it into the current allocation's canonical `unit_updates`, dry-run them automatically, and apply them through the updater. Composer supports unit/question/context updates plus confirm/drop/exclude. If it fails, correct the same decisions file and rerun it; never switch to template filling, `fill_answers.py`, or patch/fix scripts.
